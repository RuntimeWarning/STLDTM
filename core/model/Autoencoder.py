...
class FrozenAutoencoderKL(nn.Module):
    def __init__(self, ddconfig, embed_dim, pretrained_path, is_train):
        super().__init__()
...
def get_model(pretrained_path, is_train=False):
    ddconfig = dict(
        double_z=True,
        z_channels=4,
        resolution=128,
    ...
    return FrozenAutoencoderKL(ddconfig, 4, pretrained_path, is_train)


def main():
    import os
    import torchvision.transforms as transforms
    model = get_model('lib/Autoencoder.pth')
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = model.to(device)
    T = transforms.Compose([transforms.CenterCrop(128)])
    path = 'radar_imgs'
    ...
    for fname in fnames:
        ...
        recons = recons.squeeze().cpu().numpy()[2]
        imwrite(f'recons_{fname}', (recons*255).astype(np.uint8))
